...
l_bloco = 15       # comprimento de cada bloco
...
x_out = np.sum(m_modl, axis = 0)

######## Resposta ao impulso do canal PLC
canal = loadmat('NB_0_500k.mat')['h'][0]
canal = np.concatenate((canal, np.zeros(l_modl - len(canal))))

sinal_tx = x_out                                 #sem canal
#sinal_tx = np.convolve(x_out, canal)[:l_modl]    #com canal
...
for j, noise in enumerate(sigma2):
    ...
    sinal_eq = sinal_rx          #sem canal
    #sinal_eq = np.fft.ifft(np.fft.fft(sinal_rx)/np.fft.fft(canal)) #com canal

    for i in range(m):
        z[i] = np.convolve(sinal_eq, np.conj(banco_filtros[i][::-1]))
...
plt.title(f'Sistema FMT-BPSK através de canal PLC com {m} portadoras')
