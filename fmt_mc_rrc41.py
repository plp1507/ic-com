...
SNRt = np.linspace(-2, 15, 1000) #SNR teórica
...
banco_filtros = np.zeros([m//2 + 1, len(t)], dtype = 'complex')
...
mensagem = np.random.choice(qam_const, [m//2 + 1, l_bloco*n_blocos])
...
m_modl = np.zeros([m//2 + 1, l_modl], dtype = 'complex')

for i in range(m//2 + 1):
    m_modl[i] = upfirdn(banco_filtros[i], mensagem[i], l_bloco)
    # plt.plot(np.fft.fft(m_modl[i].real), label = f'{i}')

m_modl[1:-1] *= np.sqrt(2)
...
sinal_tx = np.convolve(sinal_tx.real, canal)[:l_modl]

######## Recepção
z = np.zeros([m//2 + 1, l_modl + len(f_rrc) - 1], dtype = 'complex')
z_dn = np.zeros([m//2 + 1, n_blocos*l_bloco], dtype = 'complex')
m_r = np.zeros([m//2 + 1, n_blocos*l_bloco], dtype = 'complex')

for j, noise in enumerate(sigma2):
    w = np.sqrt(noise/2)*(np.random.randn(l_modl))
    sinal_rx = sinal_tx + w
...
plt.title(f'Sistema FMT-QAM com filtro RRC (alpha={round(alpha,2)}) com {m//2 + 1} portadoras')
